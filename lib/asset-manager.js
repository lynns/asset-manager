(function() {
  var MANIFEST_NAME, assets, async, builtAssets, expandPath, expandPaths, extractMediaType, extractRequestPaths, fs, getAbsTemplate, glob, init, manifest, manifestLocation, path, paths, precompile, resolveAsset, resolveCSS, resolveInManifest, rimraf;
    ...
    if (config.inProd && path.existsSync(manifestLocation())) {
      ...
      return expandPaths(config.paths, function() {
        ...
        for (_i = 0, _len = paths.length; _i < _len; _i++) {
          ...
          if (config.app) config.app.use(mw);
      ...
      return init(config, function() {
        ...
        return glob("{" + fullPattern + "}", options, function(er, files) {
          ...
          return async.map(files, extractRequestPaths, function(err, pathDetails) {
            var extension, match, pathDetail, _i, _len;
  ...
  resolveAsset = function(assetType, resolvers) {
    ...
    return function(route) {
      ...
      if (route.indexOf('http') === 0) return absTemplate(route);
  ...
  extractRequestPaths = function(file, cb) {
    var assetType, path, relativePath, requested, _i, _len, _results;
    _results = [];
    for (_i = 0, _len = paths.length; _i < _len; _i++) {
      path = paths[_i];
