(function() {
  var MANIFEST_NAME, assetMiddleware, async, builtAssets, compressJS, crypto, doesAssetPathExist, expandPath, expandPaths, extractMediaType, extractRequestPaths, fixCSSImagePaths, fs, generateHashedName, getAbsTemplate, glob, inProd, init, manifest, manifestLocation, mkdirRecursive, path, paths, precompile, resolveAssetPath, resolveCSS, resolveImgPath, resolveInManifest, resolvedPaths, rimraf, servePath, url;
      ...
      return init(config, function() {
        ...
        return glob("{" + fullPattern + "}", options, function(er, files) {
          ...
          return async.map(files, extractRequestPaths, function(err, pathDetails) {
            var meta, outputFilePath, pathDetail, _i, _len;
            ...
            for (_i = 0, _len = pathDetails.length; _i < _len; _i++) {
              ...
              mkdirRecursive(path.dirname(outputFilePath), 0755, function() {
                return fs.writeFile(outputFilePath, meta.content);
              });
            }
  ...
  resolveAssetPath = function(assetType) {
    ...
    return function(route) {
      if (!inProd) console.log("Processing path: " + route);
